...
import useUpdateFaq from "../../hooks/faqs/useUpdateFaq";
...
const UpdateFaq = () => {
  ...
  const { mutate: UpdateFaqMutate, isError, error, isPending } = useUpdateFaq();

  useEffect(() => {
    if (state.faq) {
      form.setFieldsValue({
        question_en: state?.faq.question?.en || "",
        question_ar: state?.faq.question?.ar || "",
      });

      setTextEn(state?.faq.answer?.en || "");
      setTextAr(state?.faq.answer?.ar || "");
    }
  }, [state, form]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 px-4 w-full">
      <div className="mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600">Update faq with multilingual support</p>
        </div>
