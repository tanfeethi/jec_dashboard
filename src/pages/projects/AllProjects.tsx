...
const AllProjects = () => {
  ...
  const { data: projectsData, isLoading } = useFetchProjects(page);
  const { mutate: deleteProjectMutate, isPending } = useDeleteProject();
  const total = projectsData?.pagination.total;
  const pageSize = projectsData?.pagination.per_page || 15;
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <div className="flex items-center gap-3">
          ...
          {/* Delete Button */}
          <button
            onClick={() => deleteProjectMutate(record.id)}
        ...
        <div className="relative px-8 py-6 bg-gradient-to-r from-slate-50 via-white to-slate-50 border-b border-slate-200/80">
          ...
          <div className="relative">
            ...
            <p className="text-slate-600 text-sm font-medium">
              Manage and organize your projects listings.
            </p>
          </div>
        </div>
