...
import useDeleteSlider from "../../hooks/sliders/useDeleteSliders";

const AllSliders = () => {
  const { data: slidersData, isLoading } = useFetchSliders();
  const navigate = useNavigate();
  const { mutate: deleteSliderMutate, isPending } = useDeleteSlider();
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() =>
              navigate(`/sliders/edit/${record.id}`, {
                state: { slider: record },
              })
            }
          ...
          {/* Delete Button */}
          <button
            onClick={() => deleteSliderMutate(record.id)}
        ...
        <div className="relative px-8 py-6 bg-gradient-to-r from-slate-50 via-white to-slate-50 border-b border-slate-200/80">
          ...
          <div className="relative">
            ...
            <p className="text-slate-600 text-sm font-medium">
              Manage and organize your sliders listings.
            </p>
          </div>
        </div>
