...
import useDeleteTestimonial from "../../hooks/testimonials/useDeleteTestimonials";

const AllTestimonials = () => {
  const navigate = useNavigate();
  const { data: testimonialsData, isLoading } = useFetchTestimonials();
  const { mutate: deleteTestimonialMutate, isPending } = useDeleteTestimonial();
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() =>
              navigate(`/testimonials/edit/${record.id}`, {
                state: { testimonial: record },
              })
            }
          ...
          {/* Delete Button */}
          <button
            onClick={() => deleteTestimonialMutate(record.id)}
          ...
          >
            ...
            <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
              Delete
            </div>
          </button>
        </div>
        ...
        <div className="relative px-8 py-6 bg-gradient-to-r from-slate-50 via-white to-slate-50 border-b border-slate-200/80">
          ...
          <div className="relative">
            ...
            <p className="text-slate-600 text-sm font-medium">
              Manage and organize your testimonials listings.
            </p>
          </div>
        </div>
